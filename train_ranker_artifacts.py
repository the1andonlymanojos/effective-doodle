...
def main() -> None:
    p = argparse.ArgumentParser(description="Train XGBRanker + scaler artifacts for the demo API.")
    p.add_argument("--out-dir", required=True, help="Where to write xgb_ranker.json, scaler.pkl, features.pkl")
    p.add_argument("--candidate-k", type=int, default=int(os.environ.get("TRAIN_CANDIDATE_K", "100")))
    p.add_argument("--train-query-limit", type=int, default=int(os.environ.get("TRAIN_QUERY_LIMIT", "10000")))
    p.add_argument("--query-batch-size", type=int, default=int(os.environ.get("TRAIN_QUERY_BATCH_SIZE", "200")))
    p.add_argument("--queries-train-path", default=os.environ.get("QUERIES_TRAIN_PATH", "queries.train.tsv"))
    p.add_argument("--qrels-train-path", default=os.environ.get("QRELS_TRAIN_PATH", "qrels.train.tsv"))
    args = p.parse_args()
    ...
    cfg = L2RConfig(
        ...
        use_embedding_features=os.environ.get("USE_EMBEDDING_FEATURES", "true").strip().lower()
        in ("1", "true", "yes", "y", "on"),
        ...
        use_prox_features=os.environ.get("USE_PROX_FEATURES", "true").strip().lower() in ("1", "true", "yes", "y", "on"),
    ).with_defaults()
    ...
    model, scaler, feat_cols, scale_cols, _ = train_ranker_from_bm25_pools_batched(
        es=es,
        cfg=cfg,
        queries_train_path=args.queries_train_path,
